...
[atype,afun,afcnstr] = my_iterchk(A);
atype='matrix';
...
if ((nargin >= 6) && ~isempty(M1))
    existM1 = 1;
    [m1type,m1fun,m1fcnstr] = my_iterchk(M1);
...
if ((nargin >= 7) && ~isempty(M2))
    existM2 = 1;
    [m2type,m2fun,m2fcnstr] = my_iterchk(M2);
...
x0iszero = (norm(x) == 0);
r = b - my_iterapp('mtimes',afun,atype,afcnstr,x,varargin{:});
...
if existM1
    r = my_iterapp('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
    if ~x0iszero
        minv_b = my_iterapp('mldivide',m1fun,m1type,m1fcnstr,b,varargin{:});
    else
        minv_b = r;
    end
...
if existM2
    r = my_iterapp('mldivide',m2fun,m2type,m2fcnstr,r,varargin{:});
    if ~x0iszero
        minv_b = my_iterapp('mldivide',m2fun,m2type,m2fcnstr,minv_b,varargin{:});
    else
        minv_b = r;
    end
    if ~all(isfinite(r)) || ~all(isfinite(minv_b))
...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
%         if (outiter-1)*inner+initer>1
        semilogy((outiter-1)*inner+initer,resvec((outiter-1)*inner+initer),'o')
        hold on
        drawnow
        title('gmres')
...
%         end
        ...
        %  Apply A to v.
        v = my_iterapp('mtimes',afun,atype,afcnstr,v,varargin{:});
        %  Apply Preconditioner.
        if existM1
            v = my_iterapp('mldivide',m1fun,m1type,m1fcnstr,v,varargin{:});
            if ~all(isfinite(v))
                flag = 2;
                break
            end
        ...
        if existM2
            v = my_iterapp('mldivide',m2fun,m2type,m2fcnstr,v,varargin{:});
            if ~all(isfinite(v))
                flag = 2;
                break
            end
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            r = b - my_iterapp('mtimes',afun,atype,afcnstr,xm,varargin{:});
            if norm(r) <= tol*n2b
                x = xm;
                flag = 0;
                iter = [outiter, initer];
            ...
            minv_r = r;
            if existM1
                minv_r = my_iterapp('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
            ...
            if existM2
                minv_r = my_iterapp('mldivide',m2fun,m2type,m2fcnstr,minv_r,varargin{:});
                if ~all(isfinite(minv_r))
                    flag = 2;
                    break
                end
...
if nargout < 2
    if restarted
        my_itermsg(sprintf('gmres(%d)',restart),tol,maxit,[outiter initer],flag,iter,relres);
    else
        my_itermsg(sprintf('gmres'),tol,maxit,initer,flag,iter(2),relres);
    end
end
