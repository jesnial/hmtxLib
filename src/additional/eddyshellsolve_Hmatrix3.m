...
for i = 1:length(f)
    ...
    % rhs
    b = [-1i*2*pi*f(i)*b0; xfix; zeros(size(B,1)-length(idfix),1)];
    ...
    if isempty(B)
        ...
        afun = @(x)[hmtx_mvm(A,x(1:A.nrow),zeros(A.nrow,1),1,1)+B.'*x(A.nrow+1:end); B*x(1:A.nrow)];
    end
    
    % anonymous function to calculate M\x, being M the preconditioner
    D = hmtx_blkdiagprecond(A);
    S = B*hmtx_mvm(D,B.',zeros(size(B.')),1,1);
    [LS,US] = lu(S);
    if isempty(B)
        Mfun = @(x)hmtx_mvm(D,x,zeros(D.nrow,1),1,1);
    else
        [LL,UU] = hmtx_ssor(A);
        Mfun = @(x)[hmtx_usolve(UU,hmtx_lsolve(LL,x(1:L.nrow))); US\(LS\x(A.nrow+1:end))];
    end
    
    % gmres solution
    [xaug,~,relres,~,resvec] = gmres(afun,b,[],errMAX,iterMAX,Mfun,[],x0);
